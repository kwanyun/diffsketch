...
def estimate(input_image,name):
    global netNetwork

    with torch.no_grad():
        assert (name in ['anime_style','contour_style']), 'Wrong sketch style for informative sketch'
        if netNetwork is None:
            netNetwork = Generator(3,1, 3).cuda()
            netNetwork.load_state_dict(torch.load(os.path.join('models/checkpoints', name, 'netG_A_latest.pth')))
            print('loaded', os.path.join('models/checkpoints', name, 'netG_A_latest.pth' ))
            netNetwork.eval()
    

        return netNetwork(input_image)
