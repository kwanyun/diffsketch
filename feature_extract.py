...
def main():
    ...
    parser.add_argument("--save_all_features", action="store_true", help="if set to true, saves all feature maps, otherwise only saves those necessary for PnP")
    ...
    parser.add_argument("--ckpt", type=str, default="sd-v1-4.ckpt", help="path to checkpoint of model")
    ...
    index = 0
    for index in range(exp_config.config.index):
        ...
        def save_sampled_img(x, i, save_path):  #x.size()=torch.Size([1, 4, 64, 64])
        ...
        with torch.no_grad():
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    x_samples_ddim = model.decode_first_stage(samples_ddim)  #torch.Size([1, 3, 512, 512])
                    x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
                    x_samples_ddim = x_samples_ddim.cpu().permute(0, 2, 3, 1).numpy()
                    x_image_torch = torch.from_numpy(x_samples_ddim).permute(0, 3, 1, 2)

                    sample_idx = 0
                    for x_sample in x_image_torch:  #torch.Size([3, 512, 512]), torch.Size([1, 3, 512, 512])
                        x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                        img = Image.fromarray(x_sample.astype(np.uint8))
